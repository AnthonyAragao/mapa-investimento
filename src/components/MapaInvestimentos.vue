...
const checkScreen = () => {
    ...
    // No mobile, menu começa fechado; no desktop, aberto
    showLayers.value = !isMobile.value;
};
...
onMounted(() => {
    ...
    // Adiciona ouvinte de redimensionamento da tela
...
<template>
    <div class="flex flex-row h-screen relative overflow-hidden">

        <!-- Botão flutuante para abrir o menu no mobile -->
        <button class="fixed top-4 left-4 z-[1000] bg-white shadow-md p-2 rounded-full cursor-pointer" @click="toggleLayers">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-[#083c94]" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
        ...
        <!-- Sidebar -->
        <aside
            class="z-[999] fixed top-0 left-0 h-screen w-72 bg-gradient-to-b from-[#083c94] via-[#173f66] to-[#2b5e85] text-white py-6 px-4 flex flex-col justify-between shadow-md transition-transform duration-300 ease-in-out"
            :class="{ '-translate-x-full': !showLayers }">
                ...
                <div class="mt-2 space-y-4 overflow-y-auto pr-1 max-h-[70vh]">
                    ...
                    <template v-for="layer in layers" :key="layer.key">
                        ...
                        <div v-if="layer.type === 'group'" class="bg-white rounded text-gray-800 border shadow-sm">
                            <details class="w-full group">
                                <summary
                                    ...
                                    {{ layer.label }}
                                    <svg class="w-4 h-4 text-[#083c94] transition-transform group-open:rotate-90"
                                ...
                                <div class="pl-4 py-2 space-y-1 bg-[#f9fafb]">
                                    <div v-for="sublayer in layer.layers" :key="sublayer.key" class="flex items-center">
                                        <input type="checkbox" :id="sublayer.key" v-model="sublayer.visible"
                                            @change="toggleLayer(sublayer.key)"
                                            class="h-4 w-4 text-[#083c94] rounded focus:ring-blue-500">
                                        <span class="inline-block w-4 h-1 mx-1"
                                            :style="{ backgroundColor: sublayer.color || '#ccc' }"></span>
                        ...
                        <div v-else class="flex items-center">
                            <input type="checkbox" :id="layer.key" v-model="layer.visible"
                            ...
                            <template v-else>
                                <span class="inline-block w-4 h-1 mx-1"
                                    :style="{ backgroundColor: layer.color || '#ccc' }"></span>
                            </template>
